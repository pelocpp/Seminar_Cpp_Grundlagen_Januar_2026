...
namespace VariantDemo {
    ...
    static void test_04() {
        ...
        auto visitor = [](const auto& elem) {
            ...
            using ElemTypeWithoutRefAndCons = std::remove_const<ElemTypeWithoutRef>::type;

            if constexpr ( std::is_same<ElemTypeWithoutRefAndCons, int>::value == true ) {
                std::println("int: {}", elem);
            }
            else if constexpr (std::is_same<ElemTypeWithoutRefAndCons, double>::value == true)
            {
                std::println("double: {}", elem);
            }
            else if constexpr (std::is_same<ElemTypeWithoutRefAndCons, std::string>::value == true)
            {
